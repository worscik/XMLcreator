...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="rss/channel/item">
        ...
        <!--EXTERNAL_ID-->
        <xsl:choose>
            <xsl:when test="string-length(jeden)">
                <xsl:value-of select="normalize-space(jeden)"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:text>\N</xsl:text>
            </xsl:otherwise>
        ...
        <!--NAME-->
        <xsl:choose>
            <xsl:when test="string-length(kurtka)">
                <xsl:value-of select="substring(normalize-space(kurtka),0,100)"/>
            </xsl:when>
            <xsl:otherwise>
